        ...
        if (http.readyState == 4) {
            ...
            if (http.responseText == "100") {
                ...
                profilename = pagedata.substring(
                            pagedata.search("<title>") + 7,
                            pagedata.search("</title>") - 8)
                profilename = profilename.split("|")
                name = profilename[0]

                profileavatar = pagedata.substring(pagedata.search(
                                                       "’s avatar") - 200,
                ...
                avatar = profileavatar[0].substring(1,
                                                    profileavatar[0].length - 1)

                stats = pagedata.substring(
                            pagedata.search(
                                '<meta name="description" content=') + 34,
                            pagedata.search(
                                '><meta property="twitter:app:name:iphone"'))
                var tracks = stats.substring(stats.search('Tracks') - 4,
                                             stats.search(
                                                 'Tracks') + 6).split(". ")[1]

                var followers = stats.substring(
                            stats.search('Tracks') + 6,
                            stats.search('Followers') + 9).split(". ")[1]
                stats = followers + "\n" + tracks
                thestats = stats
            }
        }
