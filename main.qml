...
Government::brown,Law::maroon,Living::darkgreen,Lifestyle::pink,Music::darkblue,Non-Profit::merrygold,Personal::none,Repair::darkred,Science::blue,Software::orange,Technology::darkorange,Theatre::none,Writing::none,Wellness::lightpink"
    ...
    property var slist: ["Twitter::"
        + "lightblue" + "::./img/twitter.png::twitter::@username", "Tumblr::"
        + "#343460" + "::./img/tumblr.png::tumblr::username/blogname", "Medium::"
        + cardcolor + "::./img/medium.png::medium::example.com", "WordPress::"
        + "darkgray" + "::./img/wordpress.png::wordpress::example.com", "RSS::"
        + "lightgray" + "::./img/RSS.png::blog::example.com", "SoundCloud::"
        + "orange" + "::./img/soundcloud.png::soundcloud::bandname", "Etsy::"
        + "#F56400" + "::./img/etsy.png::etsy::store", "Steemit::"
        + cardcolor + "::./img/steemit-vector-icon.png::steemit::@username"]
    ...
    onClosing: {
        ...
        if (swapopt.state == "Active") {
            ...
            notification1.themessage = "Tap again to exit"
            notificationFade.start()
        }
    }
    ...
    Timer {
        ...
        running: false
        repeat: false
        onTriggered: Scripts.save_card(userid, username, userphone, useremail,
                                       usercompany, useralias,
                                       usermotto, usermain, website1, website2,
    ...
    Timer {
        ...
        onTriggered: if (deletelist.length > deletelistOld) {
                         ...
                         if (checkcount == 3) {
                            OpenSeed.remote_delete_list(userid, listget,
                                                         deletelist)
                             listdeleter.stop()
                             deletelistOld = []
                             deletelist = []
    ...
    onConnectedChanged: Scripts.save_Stat(userid, "Impact",
                                          connected.split("><").length)
    onAcceptedChanged: Scripts.save_Stat(userid, "2-Way",
                                         accepted.split(",").length - 1)
    ...
    PositionSource {
        ...
        onSourceErrorChanged: {
            ...
            notification1.delay = 6
            notification1.visible = true
            notification1.themessage = "Location services are off. Card collection will be limited to Wifi"
    ...
    Item {
        ...
        ListView {
            ...
            onContentYChanged: if (contentY < (-1 * (parent.height * 0.3))
                                       && draggingVertically == false) {
                                   cardload.running = true
                                   notification.visible = false
                                   notification.themessage = qsTr(
                                               "No Cards found\n(Pull to Refresh, or hit the center button to try a larger search area)")
                               } else if (contentY < (-1 * (parent.height * 0.3))
            ...
            Notification {
                ...
                visible: if (cardload.running == false
                                 && passerbyGrid.count == 0) {
                             true
                         } else {
                             false
                         }
        ...
        Item {
            ...
            Image {
                ...
                source: switch (sort) {
                        case 2:
                            "./icons/view-sort-alpha-descending.svg"
                            break
                        case 3:
                            "./icons/view-sort-company-decending.svg"
                            break
                        default:
                            "./icons/view-sort-descending.svg"
                            break
                        }
            }
            ...
            MouseArea {
                ...
                onClicked: {
                    ...
                    case 1:
                        ...
                        break
                    case 2:
                        notification1.themessage = "Sort by Company"
                        notification1.visible = true
                        cardslist.clear()
                        if (selection == 0) {
                        ...
                        break
                    case 3:
                        notification1.themessage = "Sort by Category"
                        notification1.visible = true
                        cardslist.clear()
                        if (selection == 0) {
    ...
    Share {
        ...
        anchors.bottom: parent.bottom
        state: "InActive"
        title: if (where == "mycard") {
                   username.trim()
               } else {
                   currentcard_username.trim()
               }
        company: if (where == "mycard") {
                     usercompany.trim()
                 } else {
                     currentcard_companyname.trim()
                 }
        type: "send"
        message: onetimecode
        onStateChanged: if (swapopt.state == "Active") {
                            OpenSeed.onetime(currentcard_thecard, "1")
                        }
        /*  MouseArea {
        anchors.fill:parent
