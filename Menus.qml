...
Item {
    ...
    onStateChanged: {
        switch (title) {
        case "Category":
            Scripts.category_Search()
            break
        case "Category Select":
            Scripts.category_Set()
            break
        default:
            Scripts.totals()
            latestStatus = MicroBlog.latest_log("Menu", usercardNum)
            break
        }
    ...
    ListView {
        ...
        delegate: Item {
            ...
            height: if (menuitem == 1) {
                        menulistview.height * 0.15
                    } else {
                        menulistview.height * 0.09
                    }
            //anchors.horizontalCenter: parent.horizontalCenter
            Item {
                id: menuSeperator
            ...
            Item {
                ...
                Rectangle {
                    ...
                    Item {
                        id: icon
                        anchors.left: parent.left
                        anchors.leftMargin: if (menuitem != "3") {
                                                mainView.width * 0.03
                                            } else {
                                                mainView.width * 0.70
                                            }
                        ...
                        Image {
                            anchors.fill: parent
                            visible: if (menuitem != 1) {
                                         true
                                     } else {
                                         false
                                     }
                            source: if (title != "Category") {
                                        switch (menuitem) {
                                        case "0":
                                            "./icons/gps.svg"
                                            break
                                        case "2":
                                            "./icons/contact-group.svg"
                                            break
                                        case "1":
                                            "./icons/contact.svg"
                                            break
                    ...
                    Text {
                        //anchors.horizontalCenter: parent.horizontalCenter
                        anchors.verticalCenter: parent.verticalCenter
                        anchors.verticalCenterOffset: if (menuitem == 1) {
                                                          parent.width * -0.04
                                                      } else {
                                                          0
                                                      }
                        ...
                        font.bold: if (menuitem == 1) {
                                       true
                                   } else {
                                       false
                                   }
                        font.pixelSize: if (menuitem == 1) {
                                            parent.width * 0.085
                                        } else {
                                            parent.width * 0.04
                                        }
                        text: if (title != "Category") {
                                  switch (menuitem) {
                                  case "0":
                                      qsTr("Collected")
                                      break
                    ...
                    Text {
                        ...
                        text: if (title != "Category") {
                                  ...
                                  case "3":
                                      ...
                                      break
                                  case "4":
                                      qsTr("(" + totalNewMessages + ")")
                                      break
                                  case "5":
                                      qsTr("(" + requests + ")")
                                      break
                                  case "6":
                                      qsTr("(" + eventsPage.counter + ")")
                                      break
                                  default:
                                      ""
                                      break
                ...
                MouseArea {
                    ...
                    onClicked: switch (menuitem) {
                               ...
                               case "1":
                                   ...
                                   break
                               case "3":
                                   /* chatPage.state = "Available",thisWindow.state = "InActive";topBar.isActive = false;topBar.state = "chat";*/ break
                               case "4":
                               ...
                               case "5":
                                   messagePage.state = "InActive"
                                   requestPage.state = "Active"
                                   thisWindow.state = "InActive"
                                   topBar.isActive = false
                                   topBar.state = "requests"
                                   break
                               case "6":
                                   eventsPage.type = "current"
                                   eventsPage.updateevents = true
                                   eventsPage.state = "Active"
                                   thisWindow.state = "InActive"
                                   topBar.isActive = false
                                   topBar.state = "events"
                                   break
