function load_current() {
    ...
    db.transaction(function (tx) {
        ...
        tx.executeSql(
                    'CREATE TABLE IF NOT EXISTS EVENTS(id TEXT, name TEXT,userID TEXT,party TEXT,location TEXT,about MEDIUMTEXT,date INT)')

        var test = "SELECT * FROM EVENTS WHERE `date` >" + d.getTime(
                    ) + " ORDER by date"
...
function save_event(name, location, about, party, year, month, day, hour, minute) {
    ...
    db.transaction(function (tx) {
        ...
        if (name.length > 1 && location.length > 1 && party.length > 1) {
            ...
            var dataSTR = [usercardNum + "" + dd.getTime(
                               ) + "" + name, name, usercardNum, party, location, about, dd.getTime(
                               )]
            var updateStr = "UPDATE EVENTS SET name='" + name + "' location='"
                    + location + "' party='" + party + "' about='" + about + "' date="
                    + dd.getTime() + " WHERE id ='" + dataSTR[0] + "'"
            ...
            notification1.themessage = qsTr(
                        "Name and Location must be provided or you need to invite at least one other person.")
            notification1.delay = 4
            notification1.visible = true
        }
    })
...
function send_event(event) {
    ...
    db.transaction(function (tx) {
        ...
        if (pull.rows.length === 1) {
            ...
            http.send("&id=" + userid + "&devid=" + devId
                      + "&appid=" + appId + "&eventUnique=" + pull.rows.item(
                          0).id + "&title=" + pull.rows.item(0).name
                      + "&userid=" + usercardNum + "&party=" + pull.rows.item(
                          0).party + "&location=" + pull.rows.item(0).location
                      + "&about=" + pull.rows.item(0).about + "&date="
                      + pull.rows.item(0).date + "&type=sending")
        }
    })
}
...
function rsvp_event(unique, usercardNum, answer) {
    ...
    http.send("&id=" + userid + "&devid=" + devId + "&appid=" + appId + "&eventUnique=" + unique
              + "&userid=" + usercardNum + "&rsvp=" + answer + "&type=rsvp")
}
                ...
                if (webinvites.length > 1) {
                    ...
                    while (webinvites.length > inum) {
                        ...
                        db.transaction(function (tx) {
                            ...
                            var test = "SELECT * FROM EVENTS WHERE id='" + webinvites[inum].split(
                                        "::")[0] + "'"
                            ...
                            if (pull.rows.length === 0) {
                                ...
                                var insertStr = "INSERT INTO EVENTS VALUES(?,?,?,?,?,?,?)"
                                var dataSTR = [webinvites[inum].split(
                                                   "::")[0], webinvites[inum].split(
                                                   "::")[1], webinvites[inum].split(
                                                   "::")[2], webinvites[inum].split(
                                                   "::")[3], webinvites[inum].split(
                                                   "::")[4], webinvites[inum].split(
                                                   "::")[5], webinvites[inum].split(
                                                   "::")[6]]

                                tx.executeSql(insertStr, dataSTR)
                            } else {

                                var updateStr = "UPDATE EVENTS SET name='" + webinvites[inum].split(
                                            "::")[1] + "', location='" + webinvites[inum].split(
                                            "::")[4] + "', \
party='" + webinvites[inum].split
("::")[3] + "', about='" + webinvites[inum].split
("::")[5] + "', date=" + webinvites[inum].split
("::")[6] + " WHERE id ='" + webinvites[inum].split("::")[0] + "'"
    ...
    http.send("&id=" + userid + "&devid=" + devId + "&appid=" + appId
              + "&userid=" + usercardNum + "&date=" + d.getTime(
                  ) + "&type=retrieve")
}
...
function check_invites() {
    ...
    db.transaction(function (tx) {
        ...
        var test = "SELECT * FROM EVENTS WHERE userID !=" + usercardNum
                + " AND `date` >" + d.getTime() + " ORDER by date"
        var pull = tx.executeSql(test)
            ...
            while (pnum < party.length) {
                ...
                if (party[pnum].split(":")[0] === usercardNum) {
                    ...
                    if (party[pnum].split(":")[1] === "0") {
                        ...
                        var test1 = "SELECT * FROM SavedCards WHERE id='" + pull.rows.item(
                                    num).userID + "'"
