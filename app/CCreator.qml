...
Item {
    ...
    states: [
        State {
            ...
            PropertyChanges {
                target:fbackground
                currentIndex:if(cardindex == 0) {fbgnum} else {fbgnum1}
            }
            PropertyChanges {
                target:fsymbol
                currentIndex:if(cardindex == 0) {fsymbolnum} else {fsymbolnum1}
            }
            PropertyChanges {
                target:ftext
                currentIndex:if(cardindex == 0) {ftextnum} else {ftextnum1}
            }
...
// height: parent.height //- units.gu(4.8)
 ...
 delegate: Item {
         ...
         Text {
             id:title
             text:if(ftitle.length >1) {ftitle} else {"Company"}
         ...
         Text {
             id:subtitle
             text:if(ftitle.length >1) {fsubtitle} else {"Username"}
...
Info {
    id:emailinfo
    width:if(window_width > mobile_width) {parent.width * 0.50} else {parent.width * 0.65}
...
Timer {
    ...
    running:false
    interval:1500
    onTriggered: {emailinfo.state = "InActive",popup.state = "InActive"}
}
